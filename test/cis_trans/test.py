...
print(f"Embedding molecule {i} with {rdmol.GetNumAtoms()} atoms (molecule graph with {mol_graph.number_of_nodes()} nodes) and {cg_mol.number_of_nodes()} CG beads...")
import networkx as nx
mol_graph = nx.Graph()
for atom in rdmol.GetAtoms():
    atom_id = atom.GetIdx()
    mol_graph.add_node(atom_id, res_id=atom.GetIntProp('res_id'), res_name=atom.GetProp('res_name'), global_res_id=atom.GetIntProp('global_res_id'))
#raise
