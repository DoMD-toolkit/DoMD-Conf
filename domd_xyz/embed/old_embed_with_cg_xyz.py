...
def generate_pos_res(molecule: Union[Chem.Mol, Chem.RWMol], cg_mol: nx.Graph, molecule_graph: nx.Graph) -> FakeConf | None:
    ...
    **Algorithm Flow:**
    ...
    Args:
        ...
        molecule_graph (nx.Graph): The full molecular graph for connectivity reference.
    ...
    #for atom_id in range(num_atoms):
        ...
        atom_id = atom.GetIdx()
        res_id = molecule_graph.nodes[atom_id]['global_res_id']
    ...
    for m_id in tqdm(cg_mol.nodes, total=len(cg_mol.nodes), desc='generating pos fragmently', disable=True):
        ...
        for i in range(count):
            ...
            for bond in atom.GetBonds():
                ...
                if molecule_graph.nodes[a.GetIdx()]["global_res_id"] not in fragment_ids:
                    # print(f"Atom {a} with residue idx {molecule.GetAtomWithIdx(a).GetIntProp('global_res_id')} not in fragment {fragment_ids}")
                    broke.add(atom_id)
                    continue
                if molecule_graph.nodes[b.GetIdx()]["global_res_id"] not in fragment_ids:
        ...
        for atom in _mh.GetAtoms():
            ...
            if molecule_graph.nodes[m_a_id]["global_res_id"] != m_id:
                continue
...
def embd(molecule: Union[Chem.Mol, Chem.RWMol], cg_molecule: nx.Graph, molecule_graph: nx.Graph, large: int = 500, custom_conf=None):
    ...
    if molecule.GetNumAtoms() > large:
        ...
        conf = generate_pos_res(molecule, cg_molecule, molecule_graph)
