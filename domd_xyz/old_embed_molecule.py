...
def embed_molecule(molecule: Union[Chem.Mol, Chem.RWMol],
                   cg_molecule: Union[nx.Graph, None] = None, molecule_graph: Union[nx.Graph, None] = None,
    ...
    for atom in molecule.GetAtoms():
        atom_id = atom.GetIdx()
        if 'res_id' not in molecule_graph.nodes[atom_id]:
            _no_res_id = True
    ...
    for atom in molecule.GetAtoms():
        if not _residue_map.get(molecule_graph.nodes[atom.GetIdx()]['res_id']):
            _residue_map[molecule_graph.nodes[atom.GetIdx()]['res_id']] = set()
        _residue_map[molecule_graph.nodes[atom.GetIdx()]['res_id']].add(atom.GetIdx())
    ...
    if cg_molecule is None:
        ...
        conf = embd(molecule, cg_molecule, molecule_graph, large=large, custom_conf=None)
